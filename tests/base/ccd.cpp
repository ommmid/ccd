...
inline double objectiveFunction(ccd::RobotStatePtr& robot_state, std::vector<Eigen::Vector4d>& target_3points)
{
    ...
    Eigen::Vector4d ee_o = robot_state->getFrames(dof - 1).second * ee_3points[0];
    Eigen::Vector4d ee_x = robot_state->getFrames(dof - 1).second * ee_3points[1];
    Eigen::Vector4d ee_z = robot_state->getFrames(dof - 1).second * ee_3points[2];

    return  (ee_o - target_3points[0]).dot(ee_o - target_3points[0]) + 
            (ee_x - target_3points[1]).dot(ee_x - target_3points[1]) + 
            (ee_z - target_3points[2]).dot(ee_z - target_3points[2]);
}
...
BOOST_AUTO_TEST_CASE(CCD2D)
{
    ...
    // Eigen::Vector4d state2_point1_in_link1_expected; state2_point1_in_link1_expected << ????;
    // for(int k = 0; k < 4; ++k)
    // {
    ...
    // // point 2 in start frame of link 1:
    // Eigen::Vector4d state2_point2_in_link1 =  robot_state_1->getPoint2InLinkStartFrame(1);
    // Eigen::Vector4d state2_point2_in_link1_expected; state2_point2_in_link1_expected << ????;
    // for(int k = 0; k < 4; ++k)
    // {
    ...
    // // point 3 in start frame of link 1:
    // Eigen::Vector4d state2_point3_in_link1 =  robot_state_1->getPoint3InLinkStartFrame(1);
    // Eigen::Vector4d state2_point3_in_link1_expected; state2_point3_in_link1_expected << ????;
    // for(int k = 0; k < 4; ++k)
    // {
